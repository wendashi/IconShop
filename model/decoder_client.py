...
def triton_model_0(triton_client, text, device, model_name='model_0', model_version='1'):
    # 检查模型是否准备好
    try:
    ...
    # 设置输入
    inputs = []
    inputs.append(httpclient.InferInput('text', text.shape, 'INT32'))  # 根据模型输入数据类型和形状设置
...
class SketchDecoder(nn.Module):
  ...
  def sample(self, n_samples, text, pixel_seq=None, xy_seq=None):
    ...
    # # for 循环外初始化！
    # onnx_path_0 = '/home/stone/Desktop/AnyFont/IconShop/onnx2triton/iconshop_0.onnx'
    # onnx_path_1 = '/home/stone/Desktop/AnyFont/IconShop/onnx2triton/iconshop_1.onnx'
    ...
    for k in range(text.shape[1] + pixlen, self.total_seq_len):
      ...
      with torch.no_grad():
        ...
        if sign == 0:
          ...
          p_pred = triton_model_0(triton_client, text, device)
          print('p_pred-0', p_pred.shape) #[8, 51, 70528])
        else:
          # ort_inputs_1 = {
          #       'pixel_seq': pixel_seq,
          #       'xy_seq': xy_seq,
          ...
          p_pred = triton_model_1(triton_client, pixel_seq, xy_seq, text, device)
          print('p_pred-1+', p_pred.shape) #[8, 562, 70528]
      ...
      if k == text.shape[1]:
        ...
        pixel_seq = torch.cat([pixel_seq, nextp_seq], 1) # 这步增加 pixel_seq[-1] seq 长度，WHY? 除了第一次，之后都在增加
